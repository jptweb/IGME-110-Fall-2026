# Signing In to Slack on Your Phone

The Slack mobile app does not put an RIT option on its sign-in screen, so the obvious buttons all
lead somewhere wrong. **Sign in with Google looks like it should work and it does not.** Neither
does typing your RIT email.

The path below is the one that works. Seven taps, once, and then you stay signed in.

> On a computer you don't need any of this. Go to
> **[rit.enterprise.slack.com](https://rit.enterprise.slack.com/)** and click the green
> **Sign in with RIT login** button.

---

## 1. Install Slack and open it

Get **Slack** from the App Store or Google Play, open it, and tap **Get Started**.

<img src="_images/slack-ios-1.png" alt="Slack welcome screen with the Get Started button" width="280">

## 2. Skip the email screen

You land on **Enter your email**. Ignore all of it: the email box, the Google button, the Apple
button. Down near the bottom, tap **Enter a workspace URL**.

<img src="_images/slack-ios-2.png" alt="Sign in screen, with Enter a workspace URL near the bottom" width="280">

## 3. You get an empty workspace field

<img src="_images/slack-ios-3.png" alt="Empty Enter a workspace URL field" width="280">

## 4. Type `rit-community` and tap Continue

Type only `rit-community`. The app fills in `.slack.com` for you.

**Type that even though it isn't our course workspace.** This is the one that hands you off to RIT
authentication. Our workspace name here won't get you signed in.

<img src="_images/slack-ios-4.png" alt="rit-community typed into the workspace URL field" width="280">

## 5. Tap Sign In with RIT Login

Now the app knows you're at RIT and shows the tiger.

<img src="_images/slack-ios-5.png" alt="Sign in to Rochester Institute of Technology with the RIT Login button" width="280">

## 6. Sign in the normal RIT way

Username (`abc1234`) and your RIT password. Leave **Stay logged in** checked.

<img src="_images/slack-ios-6.png" alt="RIT Login page asking for username and password" width="280">

## 7. Approve the Duo prompt

Same Duo push you get for everything else. When it says Success, you're in.

<img src="_images/slack-ios-7.png" alt="Duo success screen" width="280">

---

## Why `rit-community` and not our course workspace

RIT runs one Slack organization with many workspaces inside it, and `rit-community` is the one
that hands the app off to RIT authentication. Once you're through that handoff you're signed in to
the whole organization, not just that workspace.

Once you're through, tap the workspace switcher and you'll see every RIT workspace you're a member
of, our course one included. **If ours isn't in that list, message me and I'll add you.** Don't
wait until you need something.

## If you already had Slack on your phone

If the app was already installed for another RIT workspace, you don't start over. Swipe right to
open the channels menu, tap the workspace icon at the top left, then **Add a workspace** followed
by **Sign in to another workspace**. From there it's the same path: workspace URL,
`rit-community`, sign in with RIT.

## If you get stuck

- **A screen is asking you to create a password or make an account.** Wrong path. Force quit the
  app, reopen it, and start again at step 2.
- **`rit-community` isn't accepted.** Check for a typo or an autocorrected capital letter. It's all
  lowercase with a hyphen.
- **You already tapped the Google button and something half-worked.** Sign out completely, then
  start again at step 1.

RIT's own write-up of these steps is
[in the ITS knowledge base](https://help.rit.edu/sp?id=kb_article&table=kb_knowledge&sysparm_article=KB0043536).
It's shorter than this page and doesn't have the screenshots, but it's the official version if you
want to double check me.
